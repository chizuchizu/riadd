...
class CHIZUModel(LightningModule):
    ...
    def validation_epoch_end(self, input_):
        ...
        map = average_precision_score(all_true, np.round(all_pred), average=None)
...
def main(cfg):
    ...
    test_pred[["ID"] + list(cfg.base.target_cols)].to_csv(
        f"../exp9/{rand}/{rand}_{cfg.base.n_fold}_{len(cfg.base.trn_fold)}.csv", index=False)
    # oof_df.to_csv(f"../exp2/{rand}/{rand}_oof.csv", index=False)


if __name__ == "__main__":
    main(OmegaConf.load("../yaml/9.yaml"))
