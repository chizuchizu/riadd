...
class TrainDataset(Dataset):
    def __init__(self, cfg, df, transform=None, crop_transform=None, inference=False):
        self.cfg = cfg
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            augmented = self.crop_transform(image=crop)
...
class TestDataset(Dataset):
    def __init__(self, cfg, df, transform=None, crop_transform=None):
        self.cfg = cfg
        self.df = df
        self.file_names = df['ID'].values
        self.transform = transform
...
class CHIZUDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        train_dataset = TrainDataset(
            self.cfg, self.train_df,
            transform=get_transforms(self.img_sz, data="train"),
            crop_transform=get_transforms(self.cfg.model.size_cropped, data="crop")
        )
    ...
    def val_dataloader(self):
        valid_dataset = TrainDataset(
            self.cfg, self.val_df,
            transform=get_transforms(self.img_sz, data="valid"),
            crop_transform=get_transforms(self.cfg.model.size_cropped, data="crop_valid")
        )
        return DataLoader(
            valid_dataset,
            batch_size=self.batch_size,
...
class CHIZUModel_base(nn.Module):
    def __init__(self, cfg):
        ...
        if "nfnet" in self.model_name or "nf_resnet" in self.model_name or "xception" in self.model_name:
...
def train_loop(cfg, folds, fold):
    ...
    test_set = TestDataset(
        ...
        get_transforms(cfg.model.size_cropped, "crop")
    )
...
if __name__ == "__main__":
    main(OmegaConf.load("../yaml/9-crop-aug.yaml"))
